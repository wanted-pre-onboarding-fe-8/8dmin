...
export const Notice = ({ mode }: noticeProps) => {
  const content = mode === 'policy' ? privacyPolicy : privacyProvideAgreement;
  return (
    <div style={{ padding: '10%', overflow: 'scroll' }}>
      <Typography sx={{ fontWeight: 'bold' }} variant='h4'>
        {content.titleBold}
      </Typography>
      <Typography variant='h4'>{content.titleRegular}</Typography>
      <p style={{ fontWeight: 'lighter' }}>{content.description}</p>
      <p style={{ whiteSpace: 'break-spaces' }}>{content.body}</p>
    </div>
  );
};
  ...
  <Dialog fullScreen open={open} onClose={handleExit}>
    <AppBar sx={{ position: 'relative', backgroundColor: 'white' }}>
      <Toolbar>
        ...
        <Typography sx={{ ml: 2, flex: 1, color: 'gray' }} variant='h6'>
          서비스 이용약관
        </Typography>
      </Toolbar>
    </AppBar>
